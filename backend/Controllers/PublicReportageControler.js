...
export default class PublicReportageControler {
    static async createPublicReportage (req, res) {
        ...
        const images = 
        {
            imageGrandTitre: req.file ?  `${req.protocol}://${req.get('host')}/images/imagesPublicReportage/${req.file.filename}` : null
        };
        
        console.log(images);
        console.log(datePublication)
        const publicReportage = await new PublicReportage({
            titres: {
        ...
        console.log(publicReportage)
        publicReportage.save()
